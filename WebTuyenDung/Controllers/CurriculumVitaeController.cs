...
namespace WebTuyenDung.Controllers
{
    ...
    public class CurriculumVitaeController : BaseController
    {
        ...
        public async Task<IActionResult> Upload(UploadCurricilumVitaeRequest request)
        {
            ...
            {
                videoPath = await _fileService.SaveAsync(request.Video, FilePath.CurriculumTitae);
            }
        ...
        public async Task<IActionResult> Edit(int id, CurriculumVitaeDetailViewModel request)
        {
            ...
            if (request.VideoFile != null)
            {
                if (request.Video == null)
                {
                    request.Video = await _fileService.SaveAsync(request.VideoFile, FilePath.CurriculumTitae);
                }
                else
                {
                    await _fileService.ReplaceFileAsync(request.Video, request.VideoFile, FilePath.CurriculumTitae);
                }
            }
